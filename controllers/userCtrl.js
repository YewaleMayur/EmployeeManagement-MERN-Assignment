...
// APpply Employee CTRL
const applyEmployeeController = async (req, res) => {
  try {
    ...
    res.status(500).send({
      success: false,
      error,
      message: "Error WHile Applying For Employee",
    });
  }
};
...
const getAllEmployeesController = async (req, res) => {
  try {
    ...
    res.status(200).send({
      success: true,
      message: "Employee Lists Fetched Successfully",
      data: employees,
    });
  } catch (error) {
    console.log(error);
    res.status(500).send({
      success: false,
      error,
      message: "Errro WHile Fetching Employee",
    });
  }
};
...
const bookeAppointmentController = async (req, res) => {
  try {
    ...
    const user = await userModel.findOne({ _id: req.body.employeeInfo.userId });
